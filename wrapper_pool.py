...
class NSDiscreteReward(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.T == 199 and self._check_arrive_endpoint(uavPosition) == 1:
            count = count+1
        if self.T == 199 and self._check_arrive_endpoint(uavPosition) == 1 and count > 10:
            endpoint_reward = 100
        else:
            endpoint_reward = 0
        ...
        if self.T < 85:
            reward = self.rate_factor*rate_reward + self.distance_factor*distance_reward
        else :
            reward = self.distance_factor*distance_reward + self.endpoint_factor*endpoint_reward
        return obs, reward, done, truncated, info
    ...
    def _check_distance(self, pos)->float:
        ...
        distance = np.sqrt((pos[0] - self.x_max)**2 + (pos[1] - self.y_max)**2)
